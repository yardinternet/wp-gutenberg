...
const queryReducer = function ( queryState, action ) {
	switch ( action.type ) {
		...
		case REMOVE_QUERY_PARAMETER:
			const { [ action.payload ]: removedKey, ...newQueryState } =
				queryState;
			return newQueryState;

		default:
			throw new Error(
				`queryReducer does not support action type "${ action.type }".`
			);
	}
};
...
export default ( initialQueryState = {} ) => {
	...
	return {
		...
		insertParameters: ( payload ) =>
			dispatch( { type: ADD_QUERY_PARAMETER, payload } ),
		removeParameter: ( payload ) =>
			dispatch( { type: REMOVE_QUERY_PARAMETER, payload } ),
	};
};
